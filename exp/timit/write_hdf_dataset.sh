...
cat /ha/work/people/klejch/kams/kaldi/egs/timit/s5/data/train/text | cut -d' ' -f 2- | \
	sed -e 's/ /\n/g' | \
	sort | uniq | grep -v ' ' | grep -v '~' | sort > tmp_chars.txt

echo "<eol>" >> tmp_chars.txt
echo "<UNK>" >> tmp_chars.txt
...
for dt in ${datasets[*]}
do
	cat $TIMIT_S5/data/$dt/text
done | sort | uniq | cut -d' ' -f 2- > tmp_chars_all
...
compute-global-cmvn-stats.py \
	"scp:filter_scp.pl $TIMIT_S5/data/$main_train_set/wav.scp tmp_fbank40.scp|" \
...
for dt in ${datasets[*]}
do
	cat $TIMIT_S5/data/$dt/utt2spk
done | sort | uniq > ./tmp_convert_all_utt2spk
utt2spk_to_spk2utt.pl ./tmp_convert_all_utt2spk > ./tmp_convert_all_spk2utt
...
for dt in ${datasets[*]}
do
	echo $dt=/ha/work/people/klejch/kams/kaldi/egs/timit/s5/data/$dt/wav.scp
